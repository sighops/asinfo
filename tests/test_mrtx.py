...
class TestMrt(TestCase):
    """Black-box tests against ipasn.mrt's public API (parse_mrt_file), checked
    against known-good prefix -> origin-AS mappings for real RouteViews/RIPE
    RIB archives. These fixtures are deliberately truncated ("_firstMB") to
    keep them small - parse_mrt_file() is expected to stop cleanly at the
    truncation point (see the EOFError handling in mrt/tabledump.py) rather
    than raise, so only a subset of each fixture's known-good prefixes will
    actually be present in the result; every prefix that *is* present must
    match exactly.
    """

    def assert_known_origins(self, converted, expected):
        checked = 0
        for prefix, want in expected.items():
            ...
            checked += 1
            self.assertEqual(got, want, f"wrong origin for {prefix}")
        self.assertGreater(checked, 0, "none of the expected prefixes were found at all")

    def test_table_dump_v1(self):
        ...
        self.assert_known_origins(converted, {
            '3.0.0.0/8': 80,
            '4.79.181.0/24': 14780,
            '6.9.0.0/20': 668,
            '8.2.118.0/23': 13909,
    ...
    def test_table_dump_v2_ipv6(self):
    ...
    def test_skip_record_on_error_false_raises(self):
        ...
        self.assertRaises(MrtFormatError, parse_mrt_file, RIB_TD2_RECORD_FAIL_PARTDUMP)

    def test_skip_record_on_error_true_skips(self):
    ...
    def test_on_progress_callback(self):
        ...
        self.assertTrue(any("skipping" in m for m in messages))
        self.assertTrue(any(m.startswith("done:") for m in messages))
